...
class HabitViewController: UIViewController {
    ...
    var progress = CircleProgress(frame: CGRect(x: 0, y: 0, width: 80, height: 80)) // progress bar (기본 위치)
    ...
    override func viewDidAppear(_ animated: Bool) { // progress의 path가 유지되어 뜨도록 설정
    ...
    func addLabels(_ countOfHabits: Int) {
        getDataFromDB()
        switch countOfHabits { // 몇 번째 추가한 습관이냐에 따라 위치 선정
        case 1:
        ...
        if (self.countOfHabits % 6 == 0) {
            self.countOfHabits = 6
        } else {
            self.countOfHabits = self.countOfHabits % 6
        }
    ...
    @IBAction func goMyhealth(_ sender: UIButton) {
        
        let infoVC = self.storyboard?.instantiateViewController(identifier: "myhealthVC") as? MyhealthViewController
    ...
    @IBAction func addHabit(_ sender: Any) {
        ...
        if (countOfHabits % 6 == 0) { countOfHabits = 6 }
        else { countOfHabits = countOfHabits % 6 }
    ...
    func getDataFromDB() { // DB에서 데이터 읽어오기
        if (countOfHabits % 6 == 0) {
            countOfHabits = 6
        } else {
    ...
    func Data(_ index: Int) { // DB에서 특정 데이터 읽어오기
        self.ref.child("users/Habit/Habit" + "\(index)" + "/progressDegree/degree").getData { (error, snapshot) in
            if let error = error {
                print("Error getting data \(error)")
            } else if snapshot.exists() {
                print("Got data \(snapshot.value!)")
        ...
        self.ref.child("users/Habit/Habit" + "\(index)" + "/countOfHabits/count").getData { (error, snapshot) in
            if let error = error {
                print("Error getting data \(error)")
            } else if snapshot.exists() {
                print("Got data \(snapshot.value!)")
        ...
        self.ref.child("users/Habit/Habit" + "\(index)" + "/updateDate/date").getData { (error, snapshot) in
            if let error = error {
                print("Error getting data \(error)")
            } else if snapshot.exists() {
                print("Got data \(snapshot.value!)")
